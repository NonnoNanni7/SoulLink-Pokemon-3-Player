			...
			'<table class="ui table sortable selectable">' +
				'<thead>' +
					'<tr>' +
						...
						'<th class="center aligned two wide">Encounter (Player 3)</th>' +
	...
	game.locations.forEach(function(location, index) {
		...
		string += '<tr' + (location.order !== undefined ? ' class="customLocation"' : '' ) + '>' +
			...
			'<td data-sort-value="' + escapeHTML(localStorage.getItem(game.id + location.value + '-encounter2')) + '">' +
				...
				'</div>' +
			'</td>' +
			'<td data-sort-value="' + escapeHTML(localStorage.getItem(game.id + location.value + '-encounter3')) + '">' +
				'<div data-name="' + (localStorage.getItem(game.id + location.value + '-name3') ? escapeHTML(localStorage.getItem(game.id + location.value + '-name3')) : '') + '" id="' + game.id + locationValue + '-encounter3" class="ui' + (darkTheme ? ' inverted' : '') + ' fluid search selection long dropdown encounter-picker">' +
					'<input value="' + (localStorage.getItem(game.id + location.value + '-encounter3') ? escapeHTML(localStorage.getItem(game.id + location.value + '-encounter3'), true) : '') + '" name="pokemon" type="hidden">' +
					'<i class="dropdown icon"></i>' +
					'<div class="default text">Encounter (Player 3)</div>' +
					'<div class="menu"></div>' +
				'</div>' +
			'</td>' +
			...
			'<td data-sort-value="' + escapeHTML(localStorage.getItem(game.id + location.value + '-status')) + '">' +
				'<div id="' + game.id + locationValue + '-status" class="ui' + (darkTheme ? ' inverted' : '') + ' fluid selection long dropdown">' +
					...
					'<div class="menu">' +
						...
						'<div class="item" data-value="missed-p3"><i class="ban icon"></i>Missed (P3)</div>' +
						...
						'<div class="item" data-value="deceased-p3"><i class="remove user icon"></i>Deceased (P3)</div>' +
					'</div>' +
				'</div>' +
			'</td>' +
...
function clearLocation(id) {
	...
	$('#' + encounter3).dropdown('clear');
	$('#' + encounter3).closest('td').data('sortValue', '');
	...
	localStorage.removeItem(encounter3);
...
function saveData(game) {
	...
	games[game].locations.forEach(function(location) {
		...
		var encounter3 = localStorage.getItem(game + location.value + '-encounter3');
		var name = localStorage.getItem(game + location.value + '-name');
		var name2 = localStorage.getItem(game + location.value + '-name2');
		var name3 = localStorage.getItem(game + location.value + '-name3');
